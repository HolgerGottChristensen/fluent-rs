...
//! default one.
use fluent_bundle_for_carbide::{FluentArgs, FluentValue};
        ...
        if path.is_dir() {
            if let Some(name) = path.file_name() {
                if let Some(name) = name.to_str() {
                    let locale: Locale = name.parse().expect("Parsing failed.");
                    locales.push(locale);
                }
            }
        }
...
fn main() {
    ...
    // let requested: Vec<Locale> = args.get(2).map_or(vec![], |arg| {
    //     arg.split(',')
    ...
    // let default_locale: Locale = "en-US".parse().expect("Parsing failed.");
